...
```console
$ airemediate remediate [OPTIONS] PROJECT_DIR RULE_ID FILE_PATH START_LINE END_LINE START_COL END_COL LANGUAGE [STAGED_ISSUE_PUBKEY]
```
...
* `RECOMMENDATION_JSON_PARAMS`: JSON string containing remediated code and vulnerability context.
        You can pass the JSON output generated by the `remediate` command.  [required]
...
```console
$ airemediate fp_reason [OPTIONS] PROJECT_DIR RULE_ID FILE_PATH START_LINE END_LINE START_COL END_COL LANGUAGE [STAGED_ISSUE_PUBKEY]
```
...
Scan a directory using the IaC scanner. Use --raw to output unfiltered results.
...
Output:
    stdout: SymbioticScanOutput JSON (or raw Trivy JSON with --raw)
...
Scan a directory using the code security scanner. Use --raw to output unfiltered results.
