...
* `generate-guardrails`: Analyze the repository and output a...
...
Analyze the repository and output a security guardrails document.

**Usage**:

```console
$ generate-guardrails [OPTIONS] [PATH]
```
...
* `--deep`: Use a more capable (slower) model
* `--skip-sast`: Skip the SAST heuristic (faster, less precise)
* `--clear-cache`: Clear cached package classifications before running
* `--origin [CLI_STANDALONE|VCS_REMEDIATION|VCS_CHAT|IDE_REMEDIATION|IDE_CHAT|SYMBIOTIC_CODE_USER|SYMBIOTIC_CODE_AGENT|PORTAL|AGENTIC_CLI_STANDALONE|IDE_DEEP_REMEDIATION|SATELLITE]`: [Optional] Origin of the request (e.g. IDE_REMEDIATION, CLI_STANDALONE, VCS_REMEDIATION)  [default: CLI_STANDALONE]
* `--help`: Show this message and exit.
...
```console
$ airemediate remediate [OPTIONS] PROJECT_DIR RULE_ID FILE_PATH START_LINE END_LINE LANGUAGE [FINGERPRINT] [SCAN_IDEMPOTENCY_KEY] [START_COL] [END_COL]
```
...
* `[SCAN_IDEMPOTENCY_KEY]`: Idempotency key of the scan, used to avoid doublon for a scan.
* `[START_COL]`: The starting column of the vulnerability.
* `[END_COL]`: The ending column of the vulnerability.
...
* `--stream`: Enable streaming mode for live output processing
* `--origin [CLI_STANDALONE|VCS_REMEDIATION|VCS_CHAT|IDE_REMEDIATION|IDE_CHAT|SYMBIOTIC_CODE_USER|SYMBIOTIC_CODE_AGENT|PORTAL|AGENTIC_CLI_STANDALONE|IDE_DEEP_REMEDIATION|SATELLITE]`: [Optional] Origin of the request (e.g. IDE_REMEDIATION, CLI_STANDALONE)  [default: CLI_STANDALONE]
* `--help`: Show this message and exit.
...
```console
$ airemediate fp_reason [OPTIONS] PROJECT_DIR RULE_ID FILE_PATH START_LINE END_LINE START_COL END_COL LANGUAGE [FINGERPRINT] [SCAN_IDEMPOTENCY_KEY]
```
...
* `--origin [CLI_STANDALONE|VCS_REMEDIATION|VCS_CHAT|IDE_REMEDIATION|IDE_CHAT|SYMBIOTIC_CODE_USER|SYMBIOTIC_CODE_AGENT|PORTAL|AGENTIC_CLI_STANDALONE|IDE_DEEP_REMEDIATION|SATELLITE]`: [Optional] Origin of the request (e.g. IDE_REMEDIATION, VCS_REMEDIATION, CLI_STANDALONE)  [default: CLI_STANDALONE]
* `--stream`: Enable streaming mode for live output processing
* `--vuln-doc TEXT`: Vulnerability documentation
* `--code-context TEXT`: Code snippet with context around the vulnerability.
